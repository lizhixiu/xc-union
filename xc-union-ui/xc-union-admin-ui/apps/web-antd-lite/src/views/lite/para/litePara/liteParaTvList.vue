...
const handleAdd = () => {
  ...
  context.src = '/src/views/lite/para/litePara/liteParaForm.vue';
  context.actionType = 'add';
  context.param = { classId: selectedNode.value?.id };
...
const handleReorder = () => {
  const context: any = {};
  context.param = { classId: selectedNode.value?.id };
  context.postAction = '/lite/para/litePara/reorder';
...
const reloadTable = () => {
  if (tableRef.value) {
    tableRef.value.reload({ classId: selectedNode.value?.id });
  } else {
  ...
  <ColPage auto-content-height title="" v-bind="pageProps">
    ...
    <template #left="{ isCollapsed, expand }">
      ...
      >
        ...
        <XcTree
          ref="treeRef"
          url="/lite/para/liteParaClass/tree"
          label="nameTxt"
          id="id"
          title="参数树"
    ...
    <CommonTable
      ...
      api-endpoint="/lite/para/litePara/page"
