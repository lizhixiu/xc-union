...
/**
 * 基本信息表单配置
 * 包含上级部门、企业、部门编码、部门类型、部门名称、部门简称等字段
 */
  ...
  schema: [
    {
      ...
      componentProps: {
        ...
        api: () =>
          requestClient
            .post('/lite/dept/liteDept/comTreeOrgDept')
            .then((res) => res.list),
        labelField: 'name',
        valueField: 'id',
    ...
    },
    {
      ...
      componentProps: {
        // 获取部门类型的API
        api: () =>
          requestClient.post('/public/para/comList?classCode=SYS_DEPT_TYPE'),
        labelField: 'nameTxt',
        valueField: 'codeTxt',
        placeholder: '请选择部门类型',
...
async function handleSave() {
  ...
  try {
    ...
    const context = {
      ...
      saveConfig: {
        insertAction: '/lite/dept/liteDept/insert',
        updateAction: '/lite/dept/liteDept/update',
