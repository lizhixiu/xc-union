...
const handleAdd = () => {
  ...
  context.src = '/src/views/lite/post/litePost/litePostForm.vue';
...
const handleReorder = () => {
  const context: any = {};
  context.param = { pid: selectedNode.value?.id };
  context.postAction = '/lite/post/litePost/reorder';
...
const reloadTable = () => {
  if (tableRef.value) {
    tableRef.value.reload({ rowId: selectedNode.value?.id });
  } else {
...
<template>
  <ColPage auto-content-height title="" v-bind="pageProps">
    ...
    <CommonTable
      ...
      api-endpoint="/lite/post/litePost/page"
