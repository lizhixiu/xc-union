...
const formRef = {
  setValues: (values: any) => {
    try {
      ...
      const processedData = {
        ...values,
        showFlag: values?.showFlag === '1',
        keepAliveFlag: values?.keepAliveFlag === '1',
...
/**
 * 基本信息表单配置
 * 包含上级目录、导航名称、导航编码、导航别名、导航图标、导航url、组件、导航类型、导航链接类型、导航提示、打开方式、是否显示、是否缓存、系统保留、排序号、是否有效等字段
 */
  ...
  schema: [
    ...
    },
    {
      ...
      componentProps: {
        // 获取导航类型的API
        api: () =>
          requestClient.post('/public/para/comList?classCode=SYS_NAV_TYPE'),
        labelField: 'nameTxt',
        valueField: 'codeTxt',
        placeholder: '请选择导航类型',
    ...
    },
    {
      ...
      componentProps: {
        // 获取导航链接类型的API
        api: () =>
          requestClient.post('/public/para/comList?classCode=SYS_LINK_TYPE'),
        labelField: 'nameTxt',
        valueField: 'codeTxt',
        placeholder: '请选择导航链接类型',
        // 选中后同时设置 linkTypeRef 字段的值
        onChange: (value: any, option: any) => {
          baseInfoFormApi.setFieldValue('linkTypeRef', option?.label || '');
        },
        allowClear: true, // 允许清除选择
        showSearch: true, // 显示搜索框
      },
      fieldName: 'linkTypeCode',
      label: '导航链接类型',
    },
    {
      component: 'Textarea',
    ...
    },
    {
      ...
      componentProps: {
        // 获取打开方式的API
        api: () =>
          requestClient.post('/public/para/comList?classCode=SYS_NAV_OPEN_MODE'),
        labelField: 'nameTxt',
        valueField: 'codeTxt',
        placeholder: '请选择打开方式',
        allowClear: true, // 允许清除选择
    ...
    {
      component: 'Input',
      fieldName: 'linkTypeRef',
      label: '链接类型名称',
      dependencies: {
        triggerFields: ['linkTypeCode'],
        show: false, // 隐藏字段
      },
    },
  ],
...
async function handleSave() {
  ...
  try {
    ...
    const processedBaseInfoValues = {
      ...baseInfoValues,
      showFlag: baseInfoValues.showFlag ? '1' : '0',
      keepAliveFlag: baseInfoValues.keepAliveFlag ? '1' : '0',
    ...
    const context = {
      ...
      saveConfig: {
        insertAction: '/lite/nav/liteNav/insert',
        updateAction: '/lite/nav/liteNav/update',
