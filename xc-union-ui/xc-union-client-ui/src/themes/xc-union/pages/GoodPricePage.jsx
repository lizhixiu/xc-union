...
function openProductDetail(payload) {
  try {
    ...
    sessionStorage.setItem('xc_union_return_path', '/good-price');
  } catch {
    // ignore
  }
        ...
        <div className="h-full flex flex-col bg-[#fff8f7] border-0 rounded-none md:rounded-3xl md:border md:border-[#ffb7ab] md:shadow-[0_12px_30px_rgba(245,66,66,0.16)]">
          <div className="sticky top-0 z-20 p-4 md:p-5 bg-gradient-to-b from-[#FF0036] to-[#FF3141] text-white">
            <div className="flex items-center gap-2">
              {standalone && (
                <button onClick={() => navigateTo('/')} className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center shrink-0">
                  <ArrowLeft size={16} />
                </button>
              )}
