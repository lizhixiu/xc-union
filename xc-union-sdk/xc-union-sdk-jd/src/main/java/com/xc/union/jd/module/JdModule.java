...
public class JdModule {
    ...
    public <T extends AbstractResponse> T execute(@Comment(name = "params", value = "参数") Map<String, String> params) {
        ...
        if (jdConfig.isDebug()) log.info("请求报文：{}", JSONUtil.formatJsonStr(JSONUtil.toJsonStr(request)));
        // 定义响应对象
        T rsp;
        try {
            // 调用京东联盟接口
        ...
        if (jdConfig.isDebug()) log.info("返回报文：{}", JSONUtil.formatJsonStr(JSONUtil.toJsonStr(rsp)));
        // 返回响应对象
        return rsp;
    }
